...
class ConfigSvcChain(fixtures.TestWithFixtures):
    ...
    def config_st_si(self, st_name, si_name_prefix, si_count,
                     ...
                     right_vn=None, svc_type='firewall', svc_mode='transparent', flavor='contrail_flavor_2cpu', static_route=[None, None, None], ordered_interfaces=True, svc_img_name=None, st_version=1,zone=None,userdata=None):
        ...
        if zone:
            ...
            if_list=if_list, svc_mode=svc_mode, svc_scaling=svc_scaling, flavor=flavor, ordered_interfaces=ordered_interfaces, version=st_version,availability_zone_enable=True))
        else:
            st_fixture = self.useFixture(SvcTemplateFixture(
            connections=self.connections, inputs=self.inputs, domain_name=domain,
            st_name=st_name, svc_img_name=svc_img_name, svc_type=svc_type,
        ...
        for i in range(0, si_count):
            ...
            si_fixture = self.useFixture(SvcInstanceFixture(
                ...
                mgmt_vn_name=mgmt_vn, left_vn_name=left_vn, right_vn_name=right_vn, do_verify=verify_vn_ri, max_inst=max_inst, static_route=static_route,availability_zone=zone))
            if st_version == 2:
                ...
                for i in range(max_inst):
                    ...
                    if svc_mode == 'transparent':
                        svm_fixture = self.config_and_verify_vm(
                            svm_name, image_name=svc_img_name, vns=[self.trans_mgmt_vn_fixture, self.trans_left_vn_fixture, self.trans_right_vn_fixture], count=1, flavor=flavor,zone=zone,userdata=userdata)
                    else:
                        if mgmt_vn:
                            svm_fixture = self.config_and_verify_vm(
                                svm_name, image_name=svc_img_name, vns=[self.mgmt_vn_fixture, self.vn1_fixture, self.vn2_fixture], count=1, flavor=flavor,zone=zone,userdata=userdata)
                        else:
                             svm_fixture = self.config_and_verify_vm(
                                 svm_name, image_name=svc_img_name, vns=[self.vn1_fixture, self.vn2_fixture], count=1, flavor=flavor,zone=zone,userdata=userdata)
    ...
    def config_and_verify_vm(self, vm_name, vn_fix=None, image_name='ubuntu-traffic', vns=[], count=1, flavor='contrail_flavor_small',zone=None,userdata=None):
        if vns:
            vn_objs = [vn.obj for vn in vns]
            vm_fixture = self.config_vm(
                vm_name, vns=vn_objs, image_name=image_name, count=count,
                flavor=flavor,zone=zone,userdata=userdata)
        else:
            vm_fixture = self.config_vm(
    ...
    def config_vm(self, vm_name, vn_fix=None, node_name=None, image_name='ubuntu-traffic', flavor='contrail_flavor_small', vns=[], count=1,zone=None,userdata=None):
        if vn_fix:
            vm_fixture = self.useFixture(VMFixture(
                project_name=self.inputs.project_name, connections=self.connections,
                vn_obj=vn_fix.obj, vm_name=vm_name, node_name=node_name, image_name=image_name, flavor=flavor, count=count,zone=zone,userdata=userdata))
        elif vns:
            vm_fixture = self.useFixture(VMFixture(
                project_name=self.inputs.project_name, connections=self.connections,
                vm_name=vm_name, node_name=node_name, image_name=image_name, flavor=flavor, vn_objs=vns, count=count,zone=zone,userdata=userdata))

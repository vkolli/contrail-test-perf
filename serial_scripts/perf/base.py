...
class PerfBase(test_v1.BaseTestCase_v1):
    ...
    def launch_vm_on_nodes(self,hosts,family,vm_num):
        ...
        for host in hosts:
            for i in range(0,self.vm_num):
                ...
                self.vm_fixtures.append(self.useFixture(VMFixture(project_name= self.inputs.project_name, connections= self.connections, vn_objs = [ self.vn1_fixture.obj ], vm_name= vm_name,flavor='contrail_flavor_large',image_name='ubuntu-traffic1',node_name=host)))
        ...
        for host in list(set(hosts)):
            if not self.update_process_cpu1(host):
                self.logger.error("Not able to pin cpu ")
    ...
    def run_perf_tests(self,test_id,mode,proto,family,bw):
        for host in self.host_list:
            ...
            self.host_cpu_cores_0[host] = ['0-3','4-7']
    ...
    def throughput_test_40g(self,mode,proto):
        ...
        for id in range(0,len(self.encap_type)):
            ...
            for payload in  [64, 128, 512, 1400, 8000]:
                try:
                    for th_id in range(0,self.vm_num):
                        ...
                        if proto == 'TCP':
                            if not self.run_tcp_throughput_40g(self.vm_fixtures[src_id],self.vm_fixtures[dst_id],self.duration,15,payload):
                                self.logger.error("ERROR in TCP throughput test")
                                return False

                        if proto == 'UDP':
                            if not self.run_udp_throughput_40g(self.vm_fixtures[src_id],self.vm_fixtures[dst_id],self.duration,10,payload):
                                self.logger.error("ERROR in TCP throughput test")
                                return False
    ...
    def run_tcp_throughput_40g(self,src_vm_hdl,dest_vm_hdl,duration,flows,payload):
        perf_list = []
        threads = []
        thread_id = 0
    ...
    def run_udp_throughput_40g(self,src_vm_hdl,dest_vm_hdl,duration,flows,payload):
        perf_list = []
        threads = []
        thread_id = 0
        for iterate in range(0,1):
            if self.inputs.get_af() == 'v6':
                cmd = 'rm -rf start.sh; rm -rf perf.txt ; for i in {1..%s} ; do (echo \"netperf -H %s -6 -t UDP_STREAM -p 12000 -l %s  -- -m %s >> perf.txt &\" >> start.sh );done; chmod +x start.sh; sleep 5'% (flows,dest_vm_hdl.vm_ip,duration,payload)
            else:
                cmd = 'rm -rf start.sh; rm -rf perf.txt ; for i in {1..%s} ; do (echo \"netperf -H %s -t UDP_STREAM -l %s -- -m %s >> perf.txt &\" >> start.sh );done; chmod +x start.sh; sleep 5'% (flows,dest_vm_hdl.vm_ip,duration,payload)
            time.sleep(5)
            src_vm_hdl.run_cmd_on_vm(cmds=[cmd],as_sudo=True, timeout=self.timeout)
            res= "PAYLOAD : %s \n"%(payload)
            self.logger.info("Result : %s "%res)
    ...
    def get_results(self,src_vm_hdl):
        ...
        if self.proto == 'TCP':
            ...
            self.print_results(res)
        if self.proto == 'UDP':
            cmd = 'awk {\'print $4\'} perf.txt '
            src_vm_hdl.run_cmd_on_vm(cmds=[cmd],as_sudo=True, timeout=self.timeout)
            res= src_vm_hdl.return_output_cmd_dict[cmd]
            self.logger.info("Result : %s "%res)
            res=[line.strip() for line in res.split('\n') if line.strip() and 'sudo:' not in line and 'UDP' not in line and 'Socket' not in line and 'Size' not in line and 'bytes' not in line]
            del res[0::2]
            total_perf = 0
            for line in res:
                val = line.split()
                total_perf = total_perf + float(val[3])
                self.logger.info("line : %s "%line)
            perf_list.append(total_perf)
            perf_list.sort()
            time.sleep(5)
            res= "UDP %s THROUGHPUT : %s : \n"%(self.inputs.get_af(),perf_list[len(perf_list)-1])
            self.logger.info("Result : %s "%res)
            self.print_results(res)
        return True
    ...
    def update_process_cpu1(self,host):
        cmd = 'pgrep qemu'
        res = self.inputs.run_cmd_on_server(host, cmd)
        res = res.replace("\r","")
        res = res.split("\n")
        cmd = 'pgrep vhost'
        res1 = self.inputs.run_cmd_on_server(host, cmd)
        res1 = res1.replace("\r","")
        res1 = res1.split("\n")
        new_list = []
    ...
    def update_tap_mtu(self,host,tap_intf):
        cmd = 'ifconfig %s mtu 1500 ; ifconfig %s txqueuelen 100000'%(tap_intf,tap_intf)
        res = self.inputs.run_cmd_on_server(host, cmd)
        return True
    ...
    def start_v6_server(self,client_hdl):
        cmd = 'sudo ifconfig eth0 mtu 1500; sudo ethtool -L eth0 combined 4; ifconfig eth0 txqueuelen 100000; netserver -6 -p 12000 && sleep 5'
        client_hdl.run_cmd_on_vm(cmds=[cmd],as_sudo=True, timeout=self.timeout)
        res = client_hdl.return_output_cmd_dict[cmd]
        self.logger.info("Result : %s "%res)
        res= res.replace("\r", "").replace("\t","")

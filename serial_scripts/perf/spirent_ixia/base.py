...
class PerfBaseIxia(PerfBase,VerifySvcMirror):
    ...
    def setUpClass(cls):
        ...
        cls.image_flavor= { 1:'perf-ubuntu-14.04', 2:'ubuntu-perf-multique-2', 4:'ubuntu-perf-multique-4'}
    ...
    def run_spirent_perf_test(self,test_id,proto,family,cores,si,image_id):
        self.host_list = self.connections.orch.get_hosts()
        self.disable_hosts = copy.deepcopy(self.host_list)
        self.set_cpu_cores = cores
        self.set_si = si
        ...
        freemem1=0
        if image_id > 1:
            cmd='nova image-meta %s set hw_vif_multiqueue_enabled=\"true\"' %(self.image_flavor[image_id])
            print cmd
            p = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
            for line in p.stdout.readlines():
                print line,
            retval = p.wait()
        if not self.launch_svc_vms([host_vm],family,self.image_flavor[image_id],'1','2'):
            for host in self.host_list:
                self.set_nova_compute_service(host,'enable')
            self.logger.error("Failed to launch VMs")
            return False
